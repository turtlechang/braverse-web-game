...
| R3 | ~~部分 AI 路徑仍手動補記 commandLog~~ **已緩解**（2026-07-11）：`ai.ts`、`ai/battle-handler.ts`、`ai/turn-handler.ts`、`ai/random-turn-handler.ts` 全面改走 `applyGameCommand` | 低 | `play-item`／`activate-skill`／`activate-stage` 新增共用 `simulateAbilityEffects`（`src/game/ai/ability-effects.ts`）先模擬出完整 `effectTargets` 再送入 `applyGameCommand`，修復先前手動補記遺漏 `effectTargets` 導致 replay 會失真的問題；新增 `ai-replay-fidelity.test.ts` 驗證 AI 對局 `commandLog` 可經 `replayCommandLog` 重播出相同終局。過程中同時發現並修復 `commands.ts` 的 `assertNoPendingDecision` 缺口（補位帶出的新餅乾 OnPlay 未被排除在「昏厥效果待處理」阻擋之外，會讓 `skip-on-play`／`activate-skill(on-play)` 卡死）。剩餘限制：AI `refresh-deck` 洗牌仍用 `Math.random`（非種子化），該路徑的重播不保證一致，屬既有、範圍外限制 |
