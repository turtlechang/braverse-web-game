...
- 目前共有 533 項單元測試；ST4-016／017 已支援合法目標篩選、返回手牌、HP 卡送棄牌區、不提示補位，並禁止返回戰鬥區最後 1 張餅乾；ST4-018 已支援選擇抽 0～2 張、Refresh 與正確卡名；ST4-019 已支援手牌洗回牌庫後重抽相同張數及可重現 AI 洗牌；ST4-020 會要求玩家自行選擇 2 張手牌作為陷阱代價，規則層與 AI 亦完整驗證。另包含 AI 連續操作安全計數在控制權回到玩家後歸零的回歸測試，避免合法長局因整場累計達 200 步而卡住。
...
- 目前完整 `npm run test:ai:browser` 會在 1920×1080 的「玩家手牌不得覆蓋支援區」既有版面斷言失敗；相同座標已在未修改的 `aa6ef61` 基線重現，因此不是 controller／AI／effects 重構造成。`npm run test:blue:browser` 已於 1366×768、900×506 通過 ST4-012／013 與 ST4-016～020 的使用、付款、目標與決策流程；完整版面基線需另案修正。
