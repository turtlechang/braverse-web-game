...
持有者私密卡面與受規則層控制的直接登場按鈕；尚未建立牌組編輯器，也沒有為 AI 建立以 BS8
卡號為前提的策略。
...
| 從 EXTRA Deck 出牌 | 只能由回合玩家在自己的主要階段、每回合一次，從 EXTRA Deck 登場一張已滿足條件的 `extra` 或 `awakened` 餅乾。 | Rules §6-5-2-2。 | `play-extra-deck-cookie` 只接受自己的 EXTRA Deck；`canPlayExtraDeckCookie` 檢查時機、每回合一次、戰鬥區空位與逐卡條件。已轉接 BS8-005／069／090。 |
...
| BS8-005 的戰場 A/B | 持有者可見卡面並在條件成立時登場；對手只看張數；條件未滿足時不顯示登場操作。 | localhost-only `bs8-extra-deck:met`／`unmet`：Browser 實測登場、On Play 全體傷害與阻擋路徑。這不是候選逐卡 promotion 證據。 |
