...
主要敗因幾乎都是 `break-level-limit`。綠色自動策略常把低 HP、低等級餅乾補上戰鬥區，導致綠色破壞區等級快速堆高（平均 9.5），而黃色只有 7.4。

## 敗局分析

14 場敗局中，11 場綠色破壞區等級 >= 8。
...
綠色的問題比紅色更嚴重，因為綠色有更多 HP 1 的餅乾（Angel、Spinach、Bellflower、Cookiemals）。
...
綠色若把 HP 1 餅乾放成第二隻，黃色常能用攻擊或效果快速換掉。
...
不要只用「最低 HP」作為唯一目標。黃色有些低 HP 餅乾即使擊倒也對綠色幫助不大。
...
- Seed 5：綠色破壞區 11、黃色破壞區 6。綠色鋪了太多 Lv1 餅乾（Angel、Candlelight、Onion、Salt、Spinach）。
- Seed 10：綠色破壞區 10、黃色破壞區 4。綠色的 Lemon Thyme、Angel、Cookiemals 被快速換掉。
- Seed 36：綠色破壞區 11、黃色破壞區 8。綠色有 3 個 Angel Cookie 在破壞區。
...
- 回復牌不應被過早當作支援
- Supreme Whipped Cream 應保留到綠色有優勢時再使用
...
**關鍵問題：**
- 綠色破壞區有 **8 張餅乾**：3 個 Angel Cookie、2 個 Onion Cookie、Candlelight、Salt、Spinach、Lemon Thyme
- 戰鬥區只剩 1 個 Ninja Cookie，手牌只有 Bellflower 和 Red Bean
- AI 一直補低 HP 餅乾（Angel、Spinach）上戰鬥區，被黃色快速換掉

**教訓：** 綠色不應該鋪 Angel Cookie 這種 HP 1 的餅乾

### Seed 9（no-cookie-available）

**最終狀態：** 綠色破壞區 7，黃色 7，但綠色沒有餅乾了
...
**關鍵問題：**
- 綠色破壞區有 **7 張餅乾**：2 個 Angel Cookie、2 個 Cookiemals、Lemon Thyme、Onion、Red Bean
- 12 回合就結束，綠色快速堆高等級
...
**教訓：** 綠色的回復牌（Supreme Whipped Cream）沒有有效使用
