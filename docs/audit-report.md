...
| 面向 | 狀態 | 摘要 |
|---|---|---|
| 規則引擎 | ✅ 成熟 | `src/game/` 純函式引擎，typed GameCommand 指令層 + commandLog + replay，1457 項單元測試（觀測值，非固定門檻） |
...
| `npm run test` | ✅ vitest，1457 項通過（觀測值，非固定門檻）（見 §5） |
...
- `npm run lint`：✅ 通過
- `npm test`：✅ 目前測試快照 90 個測試檔、1457 項測試全數通過（此為觀測值，非固定門檻）
