...
export const BS6_SERIES_PREFIX = 'BS6-'
export const DEFAULT_LOCALE = 'en'
export const DEFAULT_OUTPUT =
  'data/candidates/official-age-of-heroes-and-kingdoms-bs6.en.json'
export const DEFAULT_INVENTORY_OUTPUT = 'docs/bs6-card-inventory.md'
...
export const selectBs6RawCards = (rawCards) => {
  ...
  return rawCards.filter((card) => {
    ...
    return cardNumber?.toUpperCase().startsWith(BS6_SERIES_PREFIX) ?? false
  })
}

export const createBs6CandidateDocument = ({
  rawCards,
  locale = DEFAULT_LOCALE,
  sourceUrl = getDatasetUrl(locale),
  importedAt = new Date().toISOString(),
}) => {
  const matchingRawCards = selectBs6RawCards(rawCards)
  if (matchingRawCards.length === 0) {
    throw new Error(
      `官方卡表沒有 ${BS6_SERIES_PREFIX} 開頭的卡片，未建立空的候選資料。`,
    )
  }
...
- 篩選規則：完整卡號以 \`${BS6_SERIES_PREFIX}\` 開頭，保留異圖／促銷變體。
...
export const runBs6CandidateImport = async ({
  locale = DEFAULT_LOCALE,
  ...
  const response = await fetchImpl(sourceUrl, {
    headers: {
      ...
      'user-agent': 'braverse-web-game-bs6-candidate-importer/0.1',
    },
  })
  if (!response.ok) {
  ...
  const inventory = createBs6InventoryMarkdown(document)
...
if (isDirectExecution) {
  try {
    const { document, outputPath, inventoryPath } = await runBs6CandidateImport(
      parseArguments(process.argv.slice(2)),
    )
    console.log(`已匯入 ${document.source.importedCount} 張 BS6 候選卡：${outputPath}`)
