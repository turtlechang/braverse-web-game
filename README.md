...
本輪亦修正 P-015 攻擊後可選代價的多段效果續接、P-016 從棄牌區移至 break 的候選與 descriptor 接線、BS4-014／BS4-080 特殊效果正規化，以及稽核驅動對 P-053／P-130 條件、P-099／P-100 FLIP 與無效果異圖的分類。最新完整 Vitest（224 檔、3,562 項；本輪覆盤 UI 新增測試通過，但 `validate-candidate-cards` 仍有 2 項 Windows registry writeback 失敗）、lint、build 均已執行；AI Browser 20／20 與牌組編輯器／本機雙瀏覽器好友房 smoke 保留為前一輪整合證據，本輪未重跑後三項。
