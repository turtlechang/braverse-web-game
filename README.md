...
- 目前完整 `npm run test:ai:browser` 會在 1920×1080 的「玩家手牌不得覆蓋支援區」既有版面斷言失敗；相同座標已在未修改的 `aa6ef61` 基線重現，因此不是 controller／AI／effects 重構造成。`npm run test:blue:browser` 已於 1366×768、900×506 的合法與不合法路徑全部通過；完整版面基線需另案修正。
