...
- 目前共有 499 項單元測試，涵蓋：牌組張數驗證、Fisher-Yates 種子重現性、開局牌組選擇與 AI 隨機牌組、開局調度、攻擊與技能能量支付、ST3-002／ST3-005／ST3-015 支援卡送棄牌區技能代價、桌面 HUD 減法配置、手牌選取動作、付款不足時隱藏非法動作與資源浮層、FLIP／TRAP 官方欄位轉換與戰鬥流程、ST2-003 Wizard Cookie 攻擊後續效果、官方標記、卡片詳情與結果提示排版、FLIP 手牌分頁、雙方依回合順序逐張選擇補位或略過、補位 OnPlay／Refresh、跨回合 OnPlay 登場窗口、AI 決策（含 faint 效果選擇）、Refresh、抽牌效果、deck-to-support、break-to-trash、物品/場景效果與完整場景合法性、When this Cookie faints、ST2-021 Pretzel Snare、ST2-001 Roguefort Cookie opponent-discard-hand、gain-hp 明確目標與 FLIP、typed GameCommand/PendingDecision pilot、統一 blocking pending guard、ST3-004 Vampire Cookie 複合 OnPlay 效果（damage + gain-hp）、ST3-017 Viney Vines support-to-trash 效果目標選取、AI 昏厥與補位優先順序、Starter Deck BLUE／PURPLE 官方範例牌組匯入與張數驗證、ST4-021 複合陷阱（攻擊減傷 + 抽牌）解析、getActingPlayerId pending controller selector、ST4-012 Werewolf Cookie discard-hand Activate 技能、ST4-013 Captain Caviar optional-cost-attack 與 inspect-deck OnPlay 效果、ST1-021 陷阱打昏攻擊者後跳過原攻擊傷害，以及對應 demo state 與 hooks 測試、五色牌組訊息、useMatchSetup／useMatchAnimations／useBattleActions、根層 ErrorBoundary、AI handler 優先分派與 effects façade 分組、支援區左界手牌扇形響應式安全內距回歸（以上方中央為共同支點向下扇形展開；畫面由左至右依序覆蓋，右側卡牌位於較高層級；六張角度 -25/-15/-5/5/15/25 度；牌背 180 度；不越過支援區左界）、PhaseRail 下一步按鈕 grid row 回歸，以及對手每張牌背內容旋轉180度，以上方中央為共同支點向下扇形展開，自然外側角度保持不變。
...
- 已知驗證限制：完整 `npm run test:ai:browser` 目前會在 1920×1080 的玩家手牌覆蓋支援區斷言失敗；相同座標已在未修改的 `aa6ef61` 重現，屬既有版面基線問題。藍牌專項瀏覽器驗證於 1366×768、900×506 的合法／不合法路徑皆通過；修正版面前不得宣稱完整 Playwright 全綠。
