...
- 正向：`/?test-state=card:BS6-084`
  - 初始手牌為 6 張，包含 Time Manipulator 與額外手牌；支援區有可支付的藍色能量，對手戰鬥區有餅乾。
  - 使用流程應依序顯示藍色能量支付、棄置至少 1 張手牌，再顯示最多 1 張對手餅乾的目標選擇；確認後造成 1 點傷害。
  - 這張卡的「手牌 5 張以下」條件是在棄牌代價完成後判定，不能在付款前把整個使用流程隱藏。
- 反向：`/?test-state=card-negative:BS6-084`
  - 支援區卡牌全部疲勞；選取手牌中的 Time Manipulator 時不應出現「使用」按鈕，證明不可支付時仍由規則層阻擋。
