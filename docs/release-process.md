...
3. **PR base `main` 但保持 open**：由維護者以另一個 agent 平台交叉驗證、Vercel preview 驗收後，自行決定合併時機。agent 不代為合併。不使用長期 develop 分支，main 即整合分支。
4. 若新工作依賴尚未合併的 PR，stack 在該 PR 分支上並在描述註明依賴順序（P1 依賴 PR 可以 stack）；互不依賴則從 main 開，且**避免修改其他 open PR 已動過的檔案**以減少衝突。
...
npm test                 # 目前基線 89 個測試檔、1455 項單元測試
npm run lint
