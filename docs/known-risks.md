...
| R15 | ~~陷阱效果只有單一共用 `targetIds`，無法支援多段效果各自選目標~~ **已解決**（2026-07-12）：`trash-to-deck` 效果新增獨立的 `trashToDeckIds` 欄位 | — | 真人試玩發現 BS2-079（「選最多 1 張對手餅乾降攻」+「選最多 5 張非 FLIP 棄牌區卡片洗回牌庫」）第二段效果因誤用第一段的目標 ID 導致例外，AI 對局矩陣卡死；先前已將 `trash-to-deck` 排除在共用 `targetIds` 之外避免例外，但效果因此變成靜默無選擇（0 張）。盤點卡池後確認全卡池唯一有此問題的陷阱只有 BS2-079，因此未採用比照物品/技能的通用 `effectTargets: string[][]` 陣列（陷阱效果迴圈有大量特例邏輯——`support-to-trash` 影響支援區、`prevent-knockout` 寫入 `pendingBattle`、`redirect-attack` 改變攻擊目標、`damage` 掛起戰鬥傷害——無法直接套用物品/技能共用的 `executeAbilityEffects`，硬套用需要相當程度的重新設計，相對於只有 1 張受影響卡不成比例）；改為比照陷阱系統既有慣例（`supportTrashIds`／`handToSupportIds`／`trashBattleCookieIds` 皆為專屬欄位）新增 `trashToDeckIds`，讓 `trash-to-deck` 有自己的目標欄位、與 `targetIds` 並存不互相覆蓋。同時修復 AI 通用 `chooseEffectTargets`（`ai.ts`）原本對 `trash-to-deck` 一律回傳空陣列的缺口（此缺口先前也讓「物品」的 trash-to-deck 效果對 AI 完全無效，不限陷阱）。已貫穿 `battle.ts`／`commands.ts`／AI `battle-handler.ts`／人類 UI（`useMatchController.ts`＋`TrapResponseModal`）四層驗證；線上對戰的陷阱 trash-to-deck 選擇 UI 尚未實作（比照既有 support-to-hand/hand-to-support 線上限制），離線對局已完整支援 |
