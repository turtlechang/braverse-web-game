...
BS8「Land of Fire & Ruin, Realm of Apathy」已完成候選資料盤點、EXTRA Deck Phase 0／1、三張可直接登場 EXTRA、兩張 Awakened 的核心 command、攻擊後效果、基礎 AI 與戰場私密檢視：171 筆記錄、125 個基礎卡號（含 15 筆 `EXTRA`）只保留在 `data/candidates/`，`validate:candidate` 已通過來源／結構驗證。依官方 Comprehensive Rules v1.8 與官方 FAQ，引擎有獨立的 0–6 張 `extraDeck`、EXTRA／Awakened 類別、每卡號四張上限、私密重排、主牌組隔離與對手資訊遮罩；BS8-005／069／090 可在自己主要階段經 `play-extra-deck-cookie` 一次直接登場。BS8-027／104 已以資料化目標條件實作 Awaken 覆蓋：保留底卡剩餘 HP 並加入 `HP+2`、保留裝備、清除已套用效果，昏厥時 Awakened 本體進 Break，底卡／HP／裝備進 Trash。Lv.1 透過通用合法指令、Lv.2 在手牌無適合登場時透過既有 Cookie 評分選取合法直接 EXTRA，均無 BS8 卡號特判；BS8-002 已完成第一張候選 serial strict contract。戰場只向持有者顯示 EXTRA 卡面，並以 localhost-only `bs8-extra-deck:met`／`unmet` Browser A/B 驗證 BS8-005 的合法登場、On Play 全體傷害及條件阻擋。尚無獨立六槽自訂牌組、固定 seed Lv.1–Lv.5 整場 AI、雙瀏覽器線上或候選逐卡 Browser gate；非 EXTRA 尚有 53 張主效果、53 項能力與 14 個攻擊 `Then` 缺口。本批維持 `inventory`、不可 promote；詳見 [BS8 卡表盤點](docs/bs8-card-inventory.md)、[效果覆蓋盤點](docs/bs8-effect-coverage.md)、[規則裁決矩陣](docs/bs8-extra-deck-ruling-matrix.md)與[EXTRA Deck 分階段方案](docs/bs8-extra-deck-plan.md)。

BS8 後續已建立與 Standard 完全隔離的候選驗收流程：候選牌組編輯器可設定 0–6 張 EXTRA（每卡號最多 4），明確標記的本機房間只接受同一候選環境並維持對手 EXTRA 遮罩；固定 seed AI 回歸與雙瀏覽器 staging Browser 檢查已通過。BS8-009、032、034、035、042、084 的 strict contract 與規則結算已補齊；整體候選 strict audit 仍為 116 verified／40 needs-review／15 blocked，BS8-043 的多候選唯一身分官方裁決與其餘逐卡 Browser gate 仍不可宣稱完成或 promote。
