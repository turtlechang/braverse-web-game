...
- 目前共有 391 項單元測試，涵蓋：牌組張數驗證、Fisher-Yates 種子重現性、開局牌組選擇與 AI 隨機牌組、開局調度、攻擊與技能能量支付、ST3-002／ST3-005／ST3-015 支援卡送棄牌區技能代價、桌面 HUD 減法配置、手牌選取動作、付款不足時隱藏非法動作與資源浮層、FLIP／TRAP 官方欄位轉換與戰鬥流程、ST2-003 Wizard Cookie 攻擊後續效果、官方標記、卡片詳情與結果提示排版、FLIP 手牌分頁、雙方依回合順序逐張選擇補位或略過、補位 OnPlay／Refresh、跨回合 OnPlay 登場窗口、AI 決策（含 faint 效果選擇）、Refresh、抽牌效果、deck-to-support、break-to-trash、物品/場景效果與完整場景合法性、When this Cookie faints、ST2-021 Pretzel Snare、ST2-001 Roguefort Cookie opponent-discard-hand、gain-hp 明確目標與 FLIP、typed GameCommand/PendingDecision pilot、統一 blocking pending guard、ST3-004 Vampire Cookie 複合 OnPlay 效果（damage + gain-hp）、ST3-017 Viney Vines support-to-trash 效果目標選取、AI 昏厥與補位優先順序、Starter Deck BLUE／PURPLE 官方範例牌組匯入與張數驗證、ST4-021 複合陷阱（攻擊減傷 + 抽牌）解析，以及 getActingPlayerId pending controller selector、支援區左界手牌扇形響應式安全內距回歸（以上方中央為共同支點向下扇形展開；畫面由左至右依序覆蓋，右側卡牌位於較高層級；六張角度 -25/-15/-5/5/15/25 度；牌背 180 度；不越過支援區左界）、PhaseRail 下一步按鈕 grid row 回歸，以及 對手每張牌背內容旋轉180度，以上方中央為共同支點向下扇形展開，自然外側角度保持不變。
